...
#define SENSOR_ERR_LOG_THROTTLE     50U     /* 失败N次，节流打印 */
#define SENSOR_COLLECT_PERIOD_MS    5U      /* 采集周期 200Hz */
 
#define BARO_VEL_LPF_ALPHA          0.05f   /* 气压速度一阶低通系数 */
#define BARO_DT_FALLBACK_S          0.02f   /* 时间戳异常时dt */
 
#define GPS_PERIOD_FALLBACK_S       0.1f    /* 无有效时间戳时按100ms */
#define GPS_FIX_MIN_SATS            6       /* 最少卫星数 */
#define GPS_FIX_MAX_EPH             2.5f    /* 最大水平精度 m */
#define GPS_LOST_MAX_SATS           4       /* 卫星数上限 */
#define GPS_LOST_EPH                3.5f    /* EPH阈值 m */
#define GPS_FIX_DEBOUNCE_CNT        10      /* 去抖次数 */
...
static volatile bool _mag_update_flag = false;
  ...
  /* create gps object*/ 
static Gps_Driv_Vel_t s_gps_drv_vel; 
static Gps_Status_t    s_gps_status;
...
static Baro_Position_t _baro_pos = {0.0f, 0.0f, 0.0f};
static Gps_Driv_Vel_t _gps_driv_vel;
static bool _gps_connected = false;

  /* baro internal states */
static struct {
    bool      inited;
    float     last_alt;      /* NED系高度 */
    uint32_t  last_ms;
} s_baro;

  /* imu channels configuration */
static Imu_Channel_t imu_channels[] = {
    {
        .name = "GYR",
        .get_data = sensor_gyr_get_calibrated_data,
        .filter_input = gyrfilter_input,
        .filter_output = gyrfilter_current,
        .raw_topic = MCN_ID(SENSOR_GYR),
        .filt_topic = MCN_ID(SENSOR_FILTER_GYR),
        .err_cnt = 0
    },
    {
        .name = "ACC",
        .get_data = sensor_acc_get_calibrated_data,
        .filter_input = accfilter_input,
        .filter_output = accfilter_current,
        .raw_topic = MCN_ID(SENSOR_ACC),
        .filt_topic = MCN_ID(SENSOR_FILTER_ACC),
        .err_cnt = 0
    },
    {
        .name = "MAG",
        .get_data = sensor_mag_get_calibrated_data,
        .filter_input = magfilter_input,
        .filter_output = magfilter_current,
        .raw_topic = MCN_ID(SENSOR_MAG),
        .filt_topic = MCN_ID(SENSOR_FILTER_MAG),
        .err_cnt = 0
    }
};
...
MCN_DEFINE(GPS_STATUS, sizeof(Gps_Status_t));
...
/* imu data process */
static void _imu_channel_process(imu_channel_t* ch)
{
    float data[3];
 
    if (ch->ready != NULL && !ch->ready()) {
        return;                                 /* 设备未挂载，静默跳过 */
    }
 
    if (ch->get_data(data) == RT_EOK) {
        ch->filter_input(data);
        mcn_publish(ch->raw_topic, data);
        mcn_publish(ch->filt_topic, ch->filter_output());
        ch->err_cnt = 0;
    } else {
        ch->err_cnt++;
        /* 节流打印：首次立即报，之后每N次报一次 */
        if (ch->err_cnt == 1 || (ch->err_cnt % SENSOR_ERR_LOG_THROTTLE) == 0) {
            Console.e(TAG, "fail to get %s data (cnt=%u)\n",
                      ch->name, (unsigned)ch->err_cnt);
        }
    }
}
...
{
    HOME_Pos home = pos_home_get();
    if (!home.gps_coordinate_set) {
        s_gps_vel.inited = false;               /* home失效则重新建立基准 */
        return;
    }
 
    Vector3f_t pos;
    gps_get_position(&pos, gps);
 
    if (!s_gps_vel.inited) {
        s_gps_vel.last_pos = pos;               /* 首帧只记录，不算速度 */
        s_gps_vel.last_ms  = gps->time_stamp;
        s_gps_vel.inited   = true;
        return;
    }
 
    float dt = (float)(gps->time_stamp - s_gps_vel.last_ms) * 1e-3f;
    if (dt <= 0.0f) {
        dt = GPS_PERIOD_FALLBACK_S;             /* 通常100ms */
    }
 
    s_gps_drv_vel.velocity.x = (pos.x - s_gps_vel.last_pos.x) / dt;
    s_gps_drv_vel.velocity.y = (pos.y - s_gps_vel.last_pos.y) / dt;
    s_gps_drv_vel.velocity.z = (pos.z - s_gps_vel.last_pos.z) / dt;
 
    s_gps_vel.last_pos = pos;
    s_gps_vel.last_ms  = gps->time_stamp;
}
 
static void _gps_update_status(const struct vehicle_gps_position_s* gps)
{
    const bool fix_ok = (gps->satellites_used >= GPS_FIX_MIN_SATS)
                     && IN_RANGE(gps->eph, 0.0f, GPS_FIX_MAX_EPH);
    const bool lost   = (gps->satellites_used <= GPS_LOST_MAX_SATS)
                     || (gps->eph > GPS_LOST_EPH);
 
    if (s_gps_status.status != GPS_AVAILABLE) {
        if (fix_ok) {
            if (++s_gps_status.fix_cnt >= GPS_FIX_DEBOUNCE_CNT) {
                s_gps_status.status = GPS_AVAILABLE;
                mcn_publish(MCN_ID(GPS_STATUS), &s_gps_status);
            }
        } else {
            s_gps_status.fix_cnt = 0;           /* 修复原版：条件中断即清零 */
        }
    }
 
    /* 判失效不去做抖：安全侧设计，尽快降级 */
    if (lost && s_gps_status.status != GPS_INAVAILABLE) {
        s_gps_status.status = GPS_INAVAILABLE;
        s_gps_status.fix_cnt = 0;
        mcn_publish(MCN_ID(GPS_STATUS), &s_gps_status);
    }
}

  /* gps data collection */
static void _process_gps(void)
{
    if (!mcn_poll(gps_node_t)) return;

    struct vehicle_gps_position_s gps;
    if (mcn_copy(MCN_ID(GPS_POSITION), gps_node_t, &gps) != RT_EOK) {
        return;
    }
    ...
    if (home.gps_coordinate_set) {
        ...
        if (!s_gps_vel.inited) {
            s_gps_vel.last_pos = pos;  /* 首次只记录位置 */
            s_gps_vel.inited = true;
        } else {
            float dt = (gps.time_stamp - s_gps_vel.last_ms) * 1e-3f;  // 如果有时间戳
            if (dt <= 0) dt = GPS_DT_FALLBACK;
            s_gps_vel.vel.x = (pos.x - s_gps_vel.last_pos.x) / dt;
            ...
        }
    }

    _update_gps_status(&gps);
}

  /* gps status update */
static void _update_gps_status(const struct vehicle_gps_position_s* gps)
{
    bool fix_ok = (gps->satellites_used >= GPS_FIX_MIN_SATS) &&
                  IN_RANGE(gps->eph, 0.0f, GPS_FIX_MAX_EPH);
    bool lost  = (gps->satellites_used <= GPS_LOST_MAX_SATS) ||
                 (gps->eph > GPS_LOST_EPH);
    ...
    if (s_gps_status.status != GPS_AVAILABLE) {
        if (fix_ok) {
            ...
            s_gps_status.fix_cnt = 0;  /* 去抖：中断即清零 */
        }
    }
...
{
    /* collect sensor data */
    for (uint32_t i = 0; i < IMU_CH_NUM; i++) {
        _imu_channel_process(&s_imu_channels[i]);
    }
    _baro_process();
    _gps_process();
